...
class CryptoTest {
    ...
    fun testCryptoRsEncode() {
        ...
        assertEquals("23YP-M8H9-FA5W-5CX9B", Crypto.rsEncode(burstCrypto.getBurstAddressFromPassphrase("").burstID.signedLongId))
        assertEquals("BTKQ-5ST6-6HAL-HKVYW", Crypto.rsEncode(burstCrypto.getBurstAddressFromPassphrase("Testing").burstID.signedLongId))
        assertEquals("4KFW-N4LS-7UVW-8AUZJ", Crypto.rsEncode(burstCrypto.getBurstAddressFromPassphrase("Burstcoin!").burstID.signedLongId))
        assertEquals("T7XD-7M3X-MB9F-38DU8", Crypto.rsEncode(burstCrypto.getBurstAddressFromPassphrase("Burst Apps Team").burstID.signedLongId))
    }

    @Test
    fun testCryptoRsDecode() {
        val burstCrypto = BurstCrypto.getInstance()
        assertEquals(burstCrypto.getBurstAddressFromPassphrase("").burstID.signedLongId, Crypto.rsDecode("23YP-M8H9-FA5W-5CX9B"))
        assertEquals(burstCrypto.getBurstAddressFromPassphrase("Testing").burstID.signedLongId, Crypto.rsDecode("BTKQ-5ST6-6HAL-HKVYW"))
        assertEquals(burstCrypto.getBurstAddressFromPassphrase("Burstcoin!").burstID.signedLongId, Crypto.rsDecode("4KFW-N4LS-7UVW-8AUZJ"))
        assertEquals(burstCrypto.getBurstAddressFromPassphrase("Burst Apps Team").burstID.signedLongId, Crypto.rsDecode("T7XD-7M3X-MB9F-38DU8"))
    }
}
