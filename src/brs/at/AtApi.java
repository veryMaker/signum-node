...
interface AtApi {
   ...
   *
   */
  long get_A1(AtMachineState state);
   ...
   * @return A2 register
   */
  long get_A2(AtMachineState state);
   ...
   * @return A3 register
   */
  long get_A3(AtMachineState state);
   ...
   * @return A4 register
   */
  long get_A4(AtMachineState state);
   ...
   * @return B1 register
   */
  long get_B1(AtMachineState state);
   ...
   * @return B2 register
   */
  long get_B2(AtMachineState state);
   ...
   * @return B3 register
   */
  long get_B3(AtMachineState state);
   ...
   * @return B4 register
   */
  long get_B4(AtMachineState state);
   ...
   *
   */
  void set_A1(long val, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void set_A2(long val, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void set_A3(long val, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void set_A4(long val, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void set_A1_A2(long val1, long val2, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void set_A3_A4(long val1, long val2, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void set_B1(long val, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void set_B2(long val, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void set_B3(long val, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void set_B4(long val, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void set_B1_B2(long val1, long val2, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void set_B3_B4(long val3, long val4, AtMachineState state);

  /**
   * sets A to zero (A being A1...4)
   * @param state AT machine state
   */
  void clear_A(AtMachineState state);

  /**
   * sets B to zero (B being B1...4)
   * @param state AT machine state
   */
  void clear_B(AtMachineState state);

  /**
   * gets A from B
   * @param state AT machine state
   */
  void copy_A_From_B(AtMachineState state);
   ...
   *
   */
  void copy_B_From_A(AtMachineState state);
   ...
   */
  long check_A_Is_Zero(AtMachineState state);
   ...
   */
  long check_B_Is_Zero(AtMachineState state);
   ...
   * @return boolean value (in a long?) if B is zero
   */
  long check_A_equals_B(AtMachineState state);

  /**
   * swap the values of A and B
   * @param state AT machine state
   */
  void swap_A_and_B(AtMachineState state);
   ...
   * @param state AT machine state
   */
  void add_A_to_B(AtMachineState state);

  /**
   * add B to A (result in A)
   * @param state AT machine state
   */
  void add_B_to_A(AtMachineState state);

  /**
   * subs A from B (result in B)
   * @param state AT machine state
   */
  void sub_A_from_B(AtMachineState state);

  /**
   * subs B from A (result in A)
   * @param state AT machine state
   */
  void sub_B_from_A(AtMachineState state);

  /**
   * multiplies A by B (result in B)
   * @param state AT machine state
   */
  void mul_A_by_B(AtMachineState state);

  /**
   * multiplies B by A (result in A)
   * @param state AT machine state
   */
  void mul_B_by_A(AtMachineState state);

  /**
   * divides A by B (result in B) *can cause a divide by zero error which would stop the machine
   * @param state AT machine state
   */
  void div_A_by_B(AtMachineState state);

  /**
   * divides B by A (result in A) *can cause a divide by zero error which would stop the machine
   * @param state AT machine state
   */
  void div_B_by_A(AtMachineState state);

  /**
   * ors A by B (result in A)
   * @param state AT machine state
   */
  void or_A_with_B(AtMachineState state);

  /**
   * ors B by A (result in B)
   * @param state AT machine state
   */
  void or_B_with_A(AtMachineState state);

  /**
   * ands A by B (result in A)
   * @param state AT machine state
   */
  void and_A_with_B(AtMachineState state);

  /**
   * ands B by A (result in B)
   * @param state AT machine state
   */
  void and_B_with_A(AtMachineState state);

  /**
   * xors A by B (result in A)
   * @param state AT machine state
   */
  void xor_A_with_B(AtMachineState state);

  /**
   * xors B by A (result in B)
   * @param state AT machine state
   */
  void xor_B_with_A(AtMachineState state);
   ...
   * @param state AT machine state
   */
  void MD5_A_to_B(AtMachineState state);
   ...
   */
  long check_MD5_A_with_B(AtMachineState state);

  /**
   * take a RIPEMD160 hash of A1..4 and put this in B1..4
   * @param state AT machine state
   */
  void HASH160_A_to_B(AtMachineState state);
   ...
   */
  long check_HASH160_A_with_B(AtMachineState state);

  /**
   * take a SHA256 hash of A1..4 abd out this in B1..4
   * @param state AT machine state
   */
  void SHA256_A_to_B(AtMachineState state);
   ...
   */
  long check_SHA256_A_with_B(AtMachineState state);
   ...
   * @param state AT machine state
   */
  long get_Block_Timestamp(AtMachineState state);

  /**
   * sets @addr to the timestamp of the AT creation block
   * @param state AT machine state
   */
  long get_Creation_Timestamp(AtMachineState state);
   ...
   */
  long get_Last_Block_Timestamp(AtMachineState state);

  /**
   * puts the block hash of the previous block in A
   * @param state AT machine state
   */
  void put_Last_Block_Hash_In_A(AtMachineState state);

  /**
   * sets A to zero/tx hash of the first tx after
   * @param state AT machine state
   */
  void A_to_Tx_after_Timestamp(long val, AtMachineState state);
   ...
   * @param state AT machine state
   */
  long get_Type_for_Tx_in_A(AtMachineState state);

  /**
   * @return bool if A is a valid tx with @addr to tx amount
   * @param state AT machine state
   */
  long get_Amount_for_Tx_in_A(AtMachineState state);

  /**
   * @return bool if A is a valid tx with @addr to the tx timestamp
   * @param state AT machine state
   */
  long get_Timestamp_for_Tx_in_A(AtMachineState state);
   ...
   * @param state AT machine state
   */
  long get_Random_Id_for_Tx_in_A(AtMachineState state);
   ...
   * @param state AT machine state
   */
  void message_from_Tx_in_A_to_B(AtMachineState state);

  /**
   * bool if A is a valid tx with B set to the tx address
   * @param state AT machine state
   */
  void B_to_Address_of_Tx_in_A(AtMachineState state);

  /**
   * set B to the address of the AT's creator
   * @param state AT machine state
   */
  void B_to_Address_of_Creator(AtMachineState state);
   ...
   * @param state AT machine state
   */
  long get_Current_Balance(AtMachineState state);
   ...
   * @param state AT machine state
   */
  long get_Previous_Balance(AtMachineState state);
   ...
   * @param state AT machine state
   */
  void send_to_Address_in_B(long val, AtMachineState state);

  /**
   * bool if B is a valid address then send it entire balance
   * @param state AT machine state
   */
  void send_All_to_Address_in_B(AtMachineState state);

  /**
   * bool if B is a valid address then send it the old balance
   * @param state AT machine state
   */
  void send_Old_to_Address_in_B(AtMachineState state);

  /**
   * bool if B is valid address then send it A as a message
   * @param state AT machine state
   */
  void send_A_to_Address_in_B(AtMachineState state);
   ...
   * @return time+minutes
   */
  long add_Minutes_to_Timestamp(long val1, long val2, AtMachineState state);
   ...
   * @param val
   */
  void set_Min_Activation_Amount(long val, AtMachineState state);
   ...
   * @param state AT machine state
   */
  void put_Last_Block_Generation_Signature_In_A(AtMachineState state);
   ...
   * @param val2
   */
  void SHA256_to_B(long val1, long val2, AtMachineState state);
}
