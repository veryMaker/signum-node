...
final class ReadMessage extends APIServlet.JsonRequestHandler {
  ...
  JsonElement processRequest(HttpServletRequest req) {
    ...
    try {
      transaction = blockchain.getTransaction(Convert.INSTANCE.parseUnsignedLong(transactionIdString));
      if (transaction == null) {
    ...
    if (message != null) {
      response.addProperty("message", message.isText() ? Convert.INSTANCE.toString(message.getMessageBytes()) : Convert.INSTANCE.toHexString(message.getMessageBytes()));
    }
      ...
      if (encryptedMessage != null) {
        ...
        if (account != null) {
          try {
            ...
            response.addProperty("decryptedMessage", encryptedMessage.isText() ? Convert.INSTANCE.toString(decrypted) : Convert.INSTANCE.toHexString(decrypted));
          } catch (RuntimeException e) {
      ...
      if (encryptToSelfMessage != null) {
        ...
        if (account != null) {
          try {
            ...
            response.addProperty("decryptedMessageToSelf", encryptToSelfMessage.isText() ? Convert.INSTANCE.toString(decrypted) : Convert.INSTANCE.toHexString(decrypted));
          } catch (RuntimeException e) {
