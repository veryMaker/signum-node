...
class FeeSuggestionCalculatorTest : AbstractUnitTest() {
    ...
    fun setUp() {
        ...
        whenever(blockchainProcessorMock!!.addListener(listenerArgumentCaptor!!.capture(), eq(Event.AFTER_BLOCK_APPLY))).thenReturn(true)
    ...
    fun getFeeSuggestion() {
        ...
        whenever(mockBlock1.transactions).thenReturn(ArrayList())
        val mockBlock2 = mock<Block>()
        whenever(mockBlock2.transactions).thenReturn(listOf(mock<Transaction>()))
        val mockBlock3 = mock<Block>()
        whenever(mockBlock3.transactions).thenReturn(listOf(mock<Transaction>()))
        val mockBlock4 = mock<Block>()
        whenever(mockBlock4.transactions).thenReturn(listOf(mock<Transaction>()))
        val mockBlock5 = mock<Block>()
        whenever(mockBlock5.transactions).thenReturn(listOf(mock<Transaction>()))

        val mockBlocksIterator = mockCollection<Block>(mockBlock1, mockBlock2, mockBlock3, mockBlock4, mockBlock5)
        whenever(blockchainStoreMock!!.getLatestBlocks(eq(5))).thenReturn(mockBlocksIterator)
        ...
        whenever(mockBlock6.transactions).thenReturn(listOf(mock<Transaction>(), mock<Transaction>(), mock<Transaction>(), mock<Transaction>()))
        val mockBlock7 = mock<Block>()
        whenever(mockBlock7.transactions).thenReturn(listOf(mock<Transaction>(), mock<Transaction>(), mock<Transaction>(), mock<Transaction>()))
        val mockBlock8 = mock<Block>()
        whenever(mockBlock8.transactions).thenReturn(listOf(mock<Transaction>(), mock<Transaction>(), mock<Transaction>(), mock<Transaction>(), mock<Transaction>()))
